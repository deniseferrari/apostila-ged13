...
```{r ch12-TH-erros-decisao, echo=FALSE, out.width = '80%', message=FALSE, warning=FALSE}
knitr::include_graphics("img/TH-erros-decisao.png")
```
...
```{r ch12-regiao-critica, echo=FALSE, out.width = '100%', message=FALSE, warning=FALSE}
knitr::include_graphics("img/regiao-critica.png")
```
...
mu.a <- 123
sd   <- 2/sqrt(3)
v.crit <- qnorm( p = 0.95, mean = 120, sd = sqrt(4/3))

lb <- mu.o - 4*sd; ub <- mu.a + 4*sd
x.o <- seq(-4, 4,length=1000)*sd + mu.o
x.a <- seq(-4, 4,length=1000)*sd + mu.a

hx.o <- dnorm(x.o, mu.o, sd)
hx.a <- dnorm(x.a, mu.a, sd)
...
i <- x.o >= v.crit & x.o <= ub
polygon(c(v.crit, x.o[i], ub), c(0, hx.o[i], 0), 
        density = 15, angle = 45,
        col = "gray") 
lines(x.o, hx.o, col = "red")

abline(h = 0)
abline(v = mu.o, col = "red", lty = "dashed")
abline(v = v.crit, col = "black", lty = "dashed")

mtext(round(mu.o, digits=1), side = 1, 
      at = mu.o, line = 0, col = "red", cex = 0.8)
mtext(expression(~ mu[o]), side = 1, 
      at = mu.o, line = 1, col = "red", cex = 0.8)
mtext(expression(~ alpha ~ "= 5%: nível de significância" ), side = 1, 
      at = v.crit + 2.6*sd, line = -3, col = "gray50", cex = 0.8)

mtext("W", side = 1,
      at = mu.o + 6*sd, line = 0, cex = 0.8)

mtext(round(v.crit, digits=1), side = 1, 
      at = v.crit, line = 0, cex = 0.8)
mtext("w* \n (valor crítico)", side = 1, 
      at = v.crit, line = 2, cex = 0.8)

mtext("Distr. amostral de W \nsob Ho", side = 3, 
      at = mu.o - 3*sd, line = -5, col = "red", cex = 0.8)
mtext("Região de Rejeição de Ho", side = 3, 
      at = v.crit + 2.5*sd, line = -5, cex = 0.8)
...
v.crit <- qnorm( p = 0.95, mean = 120, sd = sqrt(4/3))
x.p <- 124
valor.p <- pnorm(q = x.p, mean = mu.o, sd = sd, lower.tail = FALSE)
...
polygon(c(x.p, x.o[k], ub), c(0, hx.o[k], 0), 
        density = 15, angle = 135, #lwd = 5,
        col = "blue") 

abline(h = 0, lwd = 2)
abline(v = mu.o, col = "red", lty = "dashed")
abline(v = v.crit, col = "black", lty = "dashed")
abline(v = x.p, col = "blue", lty = "dashed")

mtext(round(mu.o, digits=1), side = 1, 
      at = mu.o, line = 0, col = "red")
mtext(expression(~ mu[o]), side = 1, 
      at = mu.o, line = 1, col = "red")

mtext("W", side = 1,
      at = mu.o + 6*sd, line = 0)

mtext(x.p, side = 1, 
      at = x.p, line = 0, col = "blue")
mtext(expression(~ w[obs]), side = 1,
      at = x.p, line = 1, col = "blue")

mtext(round(v.crit, digits=1), side = 1, 
      at = v.crit, line = 0)
mtext("w* \n (valor crítico)", side = 1, cex = 0.8,
      at = v.crit, line = 2)

mtext("Distr. amostral de W \nsob Ho", side = 3, cex = 0.8,
      at = mu.o - 3*sd, line = -5, col = "red")
mtext("Rej. Ho", side = 3, cex = 0.8,
      at = v.crit + 1*sd, line = -5)

mtext(expression(~ alpha ~ "= 5%" ), side = 1, 
      at = v.crit + 1*sd, line = -3, col = "gray50")
mtext("valor-p ≈ 0,03%", side = 1, cex = 0.8,
      at = v.crit + 3.5*sd, line = -2, col = "blue")
```
...
polygon(c(v.crit, x.o[i], ub), c(0, hx.o[i], 0), 
        density = 15, angle = 45,
        col = "red") 
polygon(c(lb, x.a[j], v.crit), c(0, hx.a[j], 0), 
        density = 15, angle = 135,
        col = "blue") 


lines(x.o, hx.o, col = "red")
lines(x.a, hx.a, col = "blue")

abline(h = 0, lwd = 2)
abline(v = mu.o, col = "red", lty = "dashed")
abline(v = mu.a, col = "blue", lty = "dashed")
abline(v = v.crit, col = "black", lty = "dashed")
...
mtext(round(mu.a, digits=1), side = 1, 
      at = mu.a, line = 0, col = "blue")
mtext(expression(~ mu[a]), side = 1, 
      at = mu.a, line = 1, col = "blue")
...
mtext("Distr. amostral de W \nsob Ho", side = 3, cex = 0.8,
      at = mu.o - 3*sd, line = -5, col = "red")
mtext("Distr. amostral de W \nsob Ha", side = 3, cex = 0.8,
      at = mu.a + 3*sd, line = -5, col = "blue")

mtext("Erro tipo I: Rej. Ho | HoV", side = 1, 
      at = v.crit + 3.5*sd, line = 2, cex = 0.8,
      col = "red")
mtext("Erro tipo II: n. Rej. Ho | HoF", side = 1, 
      at = v.crit - 4*sd, line = 2, cex = 0.8,
      col = "blue")

mtext(expression(~ alpha ~ "= 5%" ), side = 1,
      at = v.crit + 2*sd, line = -2, col = "red")
mtext(expression(~ beta ~ "= 17%" ), side = 1,
      at = v.crit - 2.5*sd, line = -2, col = "blue")
# mtext("valor-p ≈ 0,03%", side = 1, 
#       at = v.crit + 3*sd, line = -2, col = "blue", cex =2)
```
...
```{r ch12-TH-regiao-rej-Ho, echo=FALSE, out.width = '100%', message=FALSE, warning=FALSE}
knitr::include_graphics("img/TH-regiao-rej-Ho.png")
```
